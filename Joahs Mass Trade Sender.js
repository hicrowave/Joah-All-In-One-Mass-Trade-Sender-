...
(function () {
    ...
    const EP_KOROMONS    = 'https://www.pekora.zip/apisite/catalog/v3/search/items?category=2&limit=100&sortType=0&creatorType=1&minPrice=0&maxPrice=0&currency=3';
    const EP_DETAILS     = 'https://www.pekora.zip/apisite/catalog/v1/catalog/items/details'; // For Names and RAP
    ...
    async function fetchKoromons() {
        try {
            ...
            // 1. Fetch ALL pages from Search API (Pagination)
            console.log('Fetching all limited items (this may take a moment)...');
            while (true) {
                const url = EP_KOROMONS + (cursor ? `&cursor=${encodeURIComponent(cursor)}` : '');
                const j = await apiGet(url, false);
                const items = j.data || [];
            ...
            // 2. Enrich data with Details API (Names/RAP)
            // Split into chunks to avoid payload limits
            const chunkSize = 100;
            for (let i = 0; i < allItems.length; i += chunkSize) {
                const chunk = allItems.slice(i, i + chunkSize);
                const payload = { items: chunk.map(x => ({ itemType: 'Asset', id: x.id })) };

                try {
                    const detailsRes = await apiPost(EP_DETAILS, payload);
                    if (detailsRes && detailsRes.data) {
                        // Create a map for easy lookup
                        const detailsMap = {};
                        for (const d of detailsRes.data) detailsMap[d.id] = d;

                        for (const item of chunk) {
                            const detail = detailsMap[item.id];
                            if (detail) {
                                item.name = detail.name;
                                item.rap = detail.rap || detail.priceData?.rap || detail.price || item.price;
                            }
                        }
                    }
                } catch (e) {
                    // Continue if a chunk fails
                    console.warn('Failed to fetch details for a chunk', e);
                }
            }
            ...
            for (const it of allItems) {
                ...
                if (itemId) m[itemId] = {
                    ...
                    value: it.price || 0,
                    rap: it.rap || it.price || 0
                };
            }
            koromonsItems = m;
            console.log('Loaded ' + Object.keys(m).length + ' items.');
        } catch (e) {
            console.error(e);
        }
    }
    ...
    function showMassTradePanel() {
        ...
        (async () => {
            if (!Object.keys(koromonsItems).length) {
                mtBlastLog('Fetching full item database...', 'info');
                mtSmartLog('Fetching full item database...', 'info');
                await fetchKoromons();
            }

            if (!Object.keys(koromonsItems).length) {
                const errMsg = '<div style="padding:16px;color:#ef5350;text-align:center;font-size:12px">Failed to load items.</div>';
                ...
                mtBlastLog('Failed to load items.', 'err');
                mtSmartLog('Failed to load items.', 'err');
                return;
            }
